 ...
 */
define('QB_QUICKBOOKS_MAILTO', 'zaza@waverlycabinets.com');
...
$dsn = 'mysqli://admin:ccm321@localhost/qbwc';
...
if (!QuickBooks_Utilities::initialized($dsn))
{
	...
	if (file_exists($file))
	{
		...
		foreach (explode(';', $contents) as $sql)
		{
			...
			mysqli_query($dbh, $sql) or die(trigger_error(mysqli_error($dbh)));
		}
	}
	else
	{
...
{	
	...
	if ($Doc = $Parser->parse($errnum, $errmsg))
	{
		...
		foreach ($List->children() as $Invoice)
		{
			...
			{
				$arr[$key] = mysqli_escape_string($dbh, $value);
			}
			...
			mysqli_query($dbh, "DELETE FROM qb_example_invoice_lineitem WHERE TxnID = '" . mysqli_escape_string($dbh, $arr['TxnID']) . "' ") or die(trigger_error(mysqli_error($dbh)));
			...
			foreach ($Invoice->children() as $Child)
			{
				if ($Child->name() == 'InvoiceLineRet')
				{
					...
					{
						$lineitem[$key] = mysqli_escape_string($dbh, $value);
					}
					...
					mysqli_query($dbh, "
						...
						) ") or die(trigger_error(mysqli_error($dbh)));
				}
			}
		}
	}
 ...
 */
function WWW_quickbooks_customer_import_request($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale)
{
	// Iterator support (break the result set into small chunks)
	$attr_iteratorID = '';
	$attr_iterator = ' iterator="Start" ';
 ...
 * Handle a response from QuickBooks 
 */
function WWW_quickbooks_customer_import_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
{	
	if (!empty($idents['iteratorRemainingCount']))
	{
		// Queue up another request
...
{
	$con=mysqli_connect("localhost","admin","ccm321","qbwc");
	if (!empty($idents['iteratorRemainingCount']))
	{
		// Queue up another request
...
//	$con2=mysqli_connect("localhost","root","1234","qb_app");
	mysqli_query($con,"
		INSERT INTO
			raw_resp
		(
		...
		foreach ($List->children() as $Customer)
		{
			...
			mysqli_query($con,"
				...
				(
					...
					'" . implode("', '", array_values($arr)) . "'
				)") or die(trigger_error(mysqli_error($con)));
		}
	}
...
{	
	...
	if ($Doc = $Parser->parse($errnum, $errmsg))
	{
		...
		foreach ($List->children() as $SalesOrder)
		{
			...
			// Remove any old line items
			mysqli_query($dbh, "DELETE FROM qb_example_salesorder_lineitem WHERE TxnID = '" . mysql_real_escape_string($arr['TxnID']) . "' ") or die(trigger_error(mysql_error()));
