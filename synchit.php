...
$response = $Server->handle(true, true);
       ...
       if (!QuickBooks_Utilities::initialized($dsn)){
        ...
        function _quickbooks_customer_query_request($requestID , $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale){
			...
			global $dbh;
			$res = mysqli_query($dbh, "SELECT * FROM qw_customer WHERE id='$ID' AND quickbooks_listid='NA'");
			$row = mysqli_fetch_assoc($res);

//			$fullname = ($row['name'])? $row['name'] : $row['lname'].', '.$row['fname'];
//			$fullname = 'James Hayes';
			$fullname = $row['fname'].' '.$row['lname'];
//$email = $row['email'];
//			mysqli_query($dbh, "UPDATE qw_customer SET quickbooks_listid='N_A' WHERE id='$row[id]'");
//			mysqli_close($dbh);
...
//$xs = simplexml_load_string($xml);
		  ...
		  $content = $xml;
            $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/qbw/wcq.xml","a") or die("Unable to open file!");
            fwrite($fp, $content);
            fclose($fp);
